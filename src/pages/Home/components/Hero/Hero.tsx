import ScrollIcon from "../../../../assets/icons/mouse.svg?react";
import Testimony1 from "../../../../assets/images/customer_1.webp";
import Testimony2 from "../../../../assets/images/customer_2.webp";
import Testimony3 from "../../../../assets/images/customer_3.webp";
import HeroImage from "../../../../assets/images/hero_header.webp";
import HeroImageMobile from "../../../../assets/images/hero_header_mobile.webp";
import CpiLink from "../../../../components/CpiButton/CpiLink";
import { BOOKING_URL } from "../../../../config/links";
import "./Hero.scss";

export const Hero = () => {
  return (
    <div className="hero-wrapper">
      <picture>
        <source media="(max-width: 768px)" srcSet={HeroImageMobile} />
        <source media="(min-width: 769px)" srcSet={HeroImage} />
        <img
          className="route-header-img"
          src={HeroImage}
          alt="A hitter mid-swing during a baseball lesson at Conner Pohl Instruction in Troy, Ohio"
          fetchPriority="high"
        />
      </picture>
      <div className="hero-content">
        <div className="review-wrapper">
          <div className="review-images">
            {/* Decorative avatar stack — empty alt keeps screen readers from
                announcing three images that carry no information. */}
            <img src={Testimony1} alt="" className="image" />
            <img src={Testimony2} alt="" className="image" />
            <img src={Testimony3} alt="" className="image" />
          </div>
          <div className="review">
            <span className="testimony">
              “We’ve tried a few coaches over the years, but Swing Co. is different. Conner has a way of connecting with
              kids and breaking things...”
            </span>
            <span className="customer">- Hayden Westfall</span>
          </div>
        </div>
        <div className="hero-header">
          <h1>
            COMPETITIVE PERFORMANCE
            <br />
            INSTRUCTION
          </h1>
          <div className="call-to-action">
            <div id="book-now">
              <CpiLink label="Book now" href={BOOKING_URL} className="cpi-button light" newTab={true} />
            </div>
            <div id="contact">
              <CpiLink label="Contact" href="/contact" className="cpi-button light" />
            </div>
            <div id="schedule-session">
              <CpiLink label="Schedule a Session" href={BOOKING_URL} className="cpi-button light" newTab={true} />
            </div>
          </div>
        </div>
        <div className="scroll-accent">
          Scroll down
          <ScrollIcon id="arrow-icon" />
        </div>
      </div>
    </div>
  );
};
